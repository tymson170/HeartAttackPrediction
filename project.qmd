...
Wyraźnie widać, że rozkład mężczyzn z atakiem serca (lewy dolny histogram) przypomina rozkład normalny.
Natomiast pozostałe 3 wykresy są do siebie podobne i także przypominają rozkład normalny, tylko z o wiele mniejszą kurtozą.
```{r}
ggplot(df, aes(x = Age)) +
...
df %>% filter(Result=="Positive")%>%  ggplot(aes(y=Age,fill=Gender))+
  geom_boxplot()
```
Test na równość średnich wieku kobiet i mężczyzn w zbiorze:
```{r}
