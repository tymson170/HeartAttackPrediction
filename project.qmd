...
table2 <- table(Rzeczywistość=test$Result, Przewidziano=model_test[,'model_test'])
...
```{r}
modelsummary(list(model, model4))
```
...
Modele stworzone bez zmiennych CK.MB i Troponiny mają o wiele gorszą dokładność. Jedyna zmienna, która w istotny sposób wpływa na wynik jest wiek, ale nie jest ona w stanie przewidzieć czy pacjent ma zawał serca tak dobrze jak w modelu z zmiennymi CK.MB i Troponin.

Być może gdybyśmy w dostępnych danych mieli takie wartości jak: - czy dana osoba pali tytoń, - poziom cholesterolu, - marskość wątroby, to moglibyśmy stworzyć lepszy model, który miałby większą dokładność.
...
Lekarze stwierdzając świeży zawał mięśnia sercowego, stosują wartość odcięcia (czyli wartość powyżej 99 centyla zakresu referencyjnego). Dla troponiny ta wartość wacha się od $0,01 \mu g/l$ do $0,08 \mu g/l$ w zależności od producenta testu. W naszym modelu wartość, od której decydujemy czy ktoś ma zawał dla troponiny wynosi `r`. (\~0.015) \# TODO wpisac wartosc z modelu i dopisac jak sie ma do stosowanego przez lekarzy i dac bibliografie do tego.

CKMB \~6.5 (w sieci od 5 (znalesc zrodlo))

3.  Używając technik wizualizacji danych wykryć wzorce i nietypowe zachowania w zbiorze danych, a także lepiej zrozumieć rozkłady poszczególnych zmiennych.

Wizualizacja danych pozwoliła nam zauważyć, że ...
