...
```{r}
plot(density(df$`Heart Rate`), main = "Rozkład zmiennej Heart Rate", xlab = "Tętno", ylab = "Gęstość")

# library(ggplot2)
# library(plotly)
# wykres <- ggplot(df, aes(x = `Heart Rate`)) + geom_density() + xlab("Heart # rate") + ylab("Gęstość")
...
```{r}
#pairplot with fill as result
library(GGally)
ggpairs(df, columns = c(1, 3, 4, 5, 6, 7, 8), aes(color = Result))
```
...
testt <- t.test(wiek_mezczyzn, wiek_kobiet)
```

Wynik testu t na równość średnich wieku mężczyzn i kobiet w zbiorze wynosi `r testt$p.value %>% round(5)` i jest poniżej poziomu istotności 0.05, więc odrzucamy hipotezę o równości średnich wieku mężczyzn i kobiet w zbiorze. Wynika z tego, że w naszym zbiorze mężczyźni są starsi niż kobiety.
...
```{r}
p <- ggplot(df, aes(x = CK.MB, y = Troponin, col = factor(Result))) + geom_point(alpha = 0.5) + scale_color_manual(values = c("1" = "red", "0" = "blue")) + labs(x = "CK.MB", y = "Troponin", col = "Result") + theme_minimal()
