...
```
Zmienna Heart Rate zawiera watości maksymalne 1111, które są błędem. Sprawdzimy dokładniej tę zmienną.
...
age_breaks <- seq(14, 105, 3)
df_sum$AgeGroup <- cut(df_sum$Age, breaks = age_breaks, labels = c(1:30), include.lowest = TRUE)

df_sum$AgeGroup <- as.numeric(df_sum$AgeGroup)


df_pop <- df_sum %>% 
  mutate(AgeLowerLimit = 14 + 3*(AgeGroup-1)) %>% 
  group_by(AgeGroup) %>% 
  mutate(Population = n())

df_pop$Population <- ifelse(df_pop$Gender == 0, -df_pop$Population, df_pop$Population)

ggplot(df_pop, aes(x = Population, y = factor(AgeLowerLimit), fill = Gender)) + geom_col()
...
ggplot(df_sum, aes(x = Age, y = neg_count, fill = factor(Gender))) + 
...
Hipoteza zerowa zakłada, że procent pozytywnych przypadków ataku serca jest taki sam dla obu płci. 
Hipoteza alternatywna zakłada, że procent pozytywnych przypadków ataku serca jest różny dla obu płci.
...
##### Dla przypadkow z atakiem serca
```{r warning=FALSE}
...
df %>% filter(Result == 1) %>% 
  ...
  chart.Correlation()
```
##### Dla przypadkow bez ataku serca
```{r warning=FALSE}
df %>% filter(Result == 0) %>% 
  select(c(-Result,-Gender)) %>%
  chart.Correlation()
