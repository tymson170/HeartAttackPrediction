...
```{r}
library(rstatix)
df %>% get_summary_stats(show = c("min", "median","mean", "max")) %>% select(-n) %>% pander()
```
...
```{r}
wiek_mezczyzn <- df %>% filter(Gender=="Male", Result=="Positive") %>% select(Age)
wiek_kobiet <- df %>% filter(Gender=="Female",Result=="Positive") %>% select(Age)
...
sprawdzic czy nam to potrzebne <https://www.statsoft.pl/textbook/stathome_stat.html?https%3A%2F%2Fwww.statsoft.pl%2Ftextbook%2Fstcoran.html> update: tam potrzeba tabliczy dwudzielczej. A my raczej takiej nie mamy jak poniżej. pewnie do usunięcia
...
+==========================+======================+======+
| Grupa\                   | (1)\                 | (2)\ |
| pracownicza              | Nie pali             | Mało |
+--------------------------+----------------------+------+
| \(1\) Starsi kierownicy\ |   4\                 |   2\ |
| (2) Młodsi kierownicy\   |   4\                 |   3\ |
| (3) Starszy personel\    | 25\                  | 10\  |
| (4) Młodszy personel\    | 18\                  | 24\  |
| (5) Sekretarki           | 10                   |   6  |
+--------------------------+----------------------+------+
| Sumy kolumn              | 61                   | 45   |
+--------------------------+----------------------+------+
